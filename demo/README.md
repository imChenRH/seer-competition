...
> 2026-08-16 V5 已完成正式重渲染：四个圆柱车轮贴地，集装箱地板顶面与世界地面齐平，集装箱扩大到 7.5×3.4×3.5 m，传送带使用 11 根横向圆柱滚筒，设施与黄线平行；13 mm 侧前方动态跟随机位把完整叉车与近距交互货物纳入同一主体包络，并避开货架、集装箱后墙和顶梁。三场景真实 Isaac 录像、USDA、JSONL、分屏片和 manifest 已在 Mac 复验，禁止碰撞与接触违规均为 0。该结论只适用于随附三次演示运行，不外推为生产安全认证。

> 2026-08-16 Fast-WAM 操作轨迹已完成：官方 `fastwam_libero_uncond_2cam224` checkpoint 在 RTX 4090 上生成全部 7D 动作，LIBERO/MuJoCo 自定义场景把任务映射为“红苹果放到黄盘”。五个固定初态、每次最多 600 步，官方 `On` 谓词结果为 2/5；第一次成功在第 596 步，第二次在第 525 步。600 步是本 Demo 的语义迁移预算，不是官方 300 步基准。五次视频、动作、状态、同步分屏片和哈希均已保存于 `fastwam-apple-plate-20260816-v1-r4`。
...
- Fast-WAM 操作：五个固定 seed 中 2 次通过原版 `env.check_success()`；页面展示第一次成功轨迹，并同时列出 5 次完整结果。它证明的是自定义 LIBERO 苹果/盘子变体，不是叉车控制或苹果专项后训练。
...
- 新时间线在生成、Isaac 每帧写入、分屏合成和 Manifest 四个边界失败关闭；车身/四轮/载荷平移和升降步长不大于 0.025 m，车身/载荷偏航和货叉倾角步长不大于 0.5°。载荷按 5 根 deck、3 根 runner 和 4 个 cargo 独立检查，只有方向正确、误差受限的支撑接触被允许。V5 正式结果：normal 1128953 次、recovery 1248809 次、intervention 651784 次候选对检查，三者禁碰数和接触违规数均为 0；水平放置误差为 0。
...
选择“Fast-WAM 验证”，发送“把红色苹果放入黄色盘子”后，页面会加载 `fastwam-apple-plate-20260816-v1-r4/presentation.mp4`。本地播放不需要 GPU；重新运行模型才需要 Linux、CUDA、LIBERO/MuJoCo 和 checkpoint。
...
6. 切换“Fast-WAM 验证”，发送苹果任务，展示模型动作、物理误差、官方成功终帧和 2/5 固定初态结果；明确 600 步不是官方基准、模型没有接管叉车。
7. 向仙工确认实机接口：底盘/门架/货叉指令、状态频率、坐标系、急停、安全 PLC、仿真模型和日志格式。

## 验证

当前 `run_demo.sh check` 在 macOS/Linux 上包含 195 项 Python 测试；Windows 上仅跳过依赖 macOS JavaScriptCore 的协议执行测试。独立 JavaScript 协议脚本包含 40 项断言。

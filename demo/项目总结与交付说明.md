...
> 更新时间：2026-08-13
>
> 当前结论：Demo 已按“局部重构”路线完成；新增显式 USD 物理挂接、双叉孔托盘、双层架构同步视频与四页签 AgentOS 控制台，可在 Mac 展示、在 Isaac Sim 环境复现。
>
...
- AgentOS 输入区展示任务指令、大脑层结构化意图、小脑层技能/Fallback 分发与审计游标；输入若与证据不一致则拒绝冒充执行。
...
- Python 自动化测试：102/102 通过。
- JavaScript 协议断言：20/20 通过。
...
浏览器打开 `http://127.0.0.1:8765`，使用四个页签切换三类运行证据和 Fast-WAM 独立验证；在输入框发送已验证任务即可同步播放双层架构视频。
...
| `tests/` | 102 项 Python 测试和 20 项前端协议断言 |

...
- 地面、仓库外壳、货架、集装箱、月台、传送带、背景载荷和障碍物八类设施执行静态碰撞合同；三份正式 USDA 各验证到 186 个碰撞 Prim。叉车和活动托盘属于运动对象，灯光、相机、标线和材质引用不属于碰撞对象。
...
|---|---|---:|---:|---:|---|---|
| normal | 完整卸货与传送带放置 | 20 | 9/9 | 0 | `COMPLETED` | 457 帧，57.125 秒 |
| recovery | 首次偏移失败，执行 `FB-F01` 后完成 | 24 | 9/9 | 1 | `COMPLETED` | 545 帧，68.125 秒 |
...
| `tests/` | 108 项 Python 测试和 29 项前端协议断言 |

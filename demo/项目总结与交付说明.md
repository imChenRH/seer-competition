...
>
> 公开分支：[`captainNemoCheng/feature/demo-visual-physics-v2`](https://github.com/captainNemoCheng/seer-competition/tree/feature/demo-visual-physics-v2)
>
> 原仓库草稿 PR：[`imChenRH/seer-competition#2`](https://github.com/imChenRH/seer-competition/pull/2)

> Fast-WAM 操作分支：[`captainNemoCheng/feature/fastwam-apple-plate`](https://github.com/captainNemoCheng/seer-competition/tree/feature/fastwam-apple-plate)。正式运行 `fastwam-apple-plate-20260816-v1-r4` 在五个固定初态中 2 次通过官方成功谓词；600 步为自定义语义迁移 Demo 预算，不是官方基准。

> Fast-WAM 独立草稿 PR：[`imChenRH/seer-competition#3`](https://github.com/imChenRH/seer-competition/pull/3)

> 2026-08-16 V5 正式重渲染说明：三场景已在 AutoDL RTX 4090 / Isaac Sim 6.0.1 使用侧前方动态跟随机位重跑并下载到 Mac 复验；原始视频、分屏视频、USDA、JSONL 与 manifest 均来自本轮正式运行。防穿模结论只限定于随附运行，不外推为生产安全认证。
...
flowchart LR
    ...
    K --> H
    J["Fast-WAM 机械臂策略"] --> L["LIBERO / MuJoCo 苹果→黄盘"]
...
- 地面、仓库外壳、货架、集装箱、月台、传送带和背景载荷执行常驻静态碰撞合同；故障障碍物仅在 intervention 可见并激活碰撞，normal/recovery 不留下隐形碰撞体。V5 normal/recovery 各验证 193 个静态碰撞 Prim，intervention 为 195 个。叉车和活动托盘属于运动对象，灯光、相机、标线和材质引用不属于碰撞对象。
...
- `2.5D_OBB_SAT_SWEEP_V3` 守卫静态几何与场景共用同一组尺寸/位姿定义，覆盖仓库墙梁、集装箱地板/侧墙/顶梁、月台、货架、传送带和背景载荷；动态集合显式包含车身、四轮、倾斜叉架、双货叉，以及与场景共用的 5 根 deck、3 根 runner 和 4 个 cargo。只有方向正确、误差不超过 1 mm 的支撑面接触可放行；车轮或载荷向下穿入地板以及其余动态—动态/动态—静态重叠均失败。
- V5 正式三场景分别完成 1128953、1248809、651784 次 Z 区间相交后、白名单过滤后的 SAT 候选对检查，禁碰数和接触违规数均为 0；normal/recovery 最小车身净距 0.213986 m，intervention 为 0.387376 m，水平放置误差为 0（允许上限 0.02 m）。
...
- 使用官方 LeRobot Fast-WAM LIBERO 两相机 checkpoint，在 RTX 4090 / CUDA 上实际生成 7D 末端与夹爪动作；没有规则控制器补动作。
- AgentOS 把“把红色苹果放入黄色盘子”映射为 checkpoint 训练域内的 `Put the bowl on the plate`，红苹果与黄盘是自定义物理/视觉对象，成功条件仍是 LIBERO `On(red_apple_1, yellow_plate_1)`。
- 场景使用两个 224×224 相机观测、20 Hz 控制；策略每 10 个动作重新调用一次。五个固定 seed 每次最多 600 步，结果为 2/5，成功发生在第 596 和 525 步。
- 第一次成功轨迹包含 597 帧、596 个动作和 60 次策略调用；`presentation.mp4` 为 2560×1080、20 fps、597 帧，左右栏使用同一帧时钟。
- 每次尝试各自的视频、动作和物理状态均由 Manifest 哈希；页面不会隐藏 3 次失败，也不会把单次成功写成泛化成功率。
- 600 步来自 300 步基线 0/5 后的显式 Demo 预算扩展，不能与官方 300 步复现实验直接比较。
- 第一次单次调用约 0.86 秒只作为早期独立技术验证保留；正式轨迹记录每次策略调用的实测延迟。
- 论文中的 190 ms 只能作为论文指标，不能写成本机实测。
- checkpoint 没有针对苹果/黄盘专项后训练，也没有控制叉车；叉车接入仍需要企业数据、动作映射和安全门控。
...
- Python 自动化测试：195/195 通过（Windows Git Bash 上依赖 macOS JavaScriptCore 的协议执行测试按预期跳过）。
- JavaScript 协议断言：40/40 通过。
...
| `tests/` | 195 项 Python 测试和 40 项前端协议断言 |
...
- 公开分支：`captainNemoCheng/feature/demo-visual-physics-v2`

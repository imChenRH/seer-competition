...
> 更新时间：2026-08-13
>
> 当前结论：Demo 已按“局部重构”路线完成，可在 Mac 展示、在 Isaac Sim 环境复现，并已形成可机器校验的正式证据包。
>
...
- 场景采用 Z-up 坐标和局部子部件变换，画面包含剖视集装箱、叉车、栈板和传送带。
- 基座、门架、货叉倾斜和载荷均写入 OpenUSD `xformOp` 时间采样。
- 载荷抓取、搬运和释放由场景相对几何关系观测，不由网页或日志常量伪造。
...
- 页面只读取已通过严格校验的正式运行目录。
- 可切换 normal、recovery、intervention。
- 同屏展示 Isaac 录像、终态、九技能执行链、Fallback、安全事件和连续审计日志。
- 不展示无法从事件推出的固定成功率或虚构延迟。
- 页面明确标注当前是运动学数字孪生 Demo，不是仙工实机或生产安全证明。
...
|---|---|---:|---:|---:|---|---|
| normal | 完整卸货与传送带放置 | 20 | 9/9 | 0 | `COMPLETED` | 409 帧，51.125 秒 |
| recovery | 首次偏移失败，执行 `FB-F01` 后完成 | 24 | 9/9 | 1 | `COMPLETED` | 497 帧，62.125 秒 |
| intervention | 三次遮挡失败，调整视角、退回并停车 | 18 | 2/9 | 2 类 | `HUMAN_REQUIRED` | 289 帧，36.125 秒 |

三段视频均为 H.264、1280×720、8 fps。每个正式运行目录包含：
...
- Python 自动化测试：80/80 通过，并将 `ResourceWarning` 提升为错误。
- JavaScript 协议断言：14/14 通过。
- 三场景 Mac 干运行及三份正式 Isaac JSONL 全部通过严格语义校验。
- 三段视频的编码、分辨率、帧率、帧数和时长经 `ffprobe` 复验。
- 三份 USDA 均包含基座、门架、货叉倾斜和载荷的时间采样。
- manifest 重算与仓库内容完全一致。
- 真实浏览器中三段视频均完整加载，三场景摘要与事件一致。
- 五轮独立严格复审后最终结论：Critical 0、Important 0、Minor 0，`Ready to merge: Yes`。
...
浏览器打开 `http://127.0.0.1:8765`，用顶部下拉框切换三个场景并播放录像。
...
1. 用 30 秒说明边界：这是证据驱动的 Isaac 运动学数字孪生，不是实机闭环。
2. 展示 normal：完整九技能、无 Fallback、载荷被放到传送带。
3. 展示 recovery：指出首次识别失败、`FB-F01` 和第二次成功来自同一事件链。
4. 展示 intervention：指出三次失败、两个观察位姿、箱外退回、速度验证和 `HUMAN_REQUIRED`。
5. 展示飞书现场任务：一个任务只启动一次 runner，审计连续并可断点恢复。
6. 单独展示 Fast-WAM `[1, 7]` 输出，明确它尚未接管叉车。
7. 向仙工确认实机模型、动作接口、状态频率、坐标系、急停、安全 PLC 和日志格式。
...
| `tests/` | 80 项 Python 测试和 14 项前端协议断言 |
